...
class Main {
    ...
    public static void main(String[] args) {
        ...
        cli._(longOpt: 'source', 'Source to read from (WSDL/HTML — required for non-pyvmomi modes)', required: false, args: 1)
        cli._(longOpt: 'dest', 'Destination path for where to write new files', required: false, args: 1)
        cli._(longOpt: 'type',
            'Type of objects to create. Valid values are one of either: dataobj, fault, enum, spbm_do, spbm_fault, spbm_enum, wsdl_do, wsdl_enum, wsdl_vimstub, pyvmomi_audit, pyvmomi_migrate, pyvmomi_mo',
            required: true, args: 1
        )
        cli.a(longOpt: 'all', 'Generate new and changed. Default is new only')
        cli._(longOpt: 'yavijava-src',    'Yavijava source root for pyvmomi_audit/pyvmomi_migrate', required: false, args: 1)
        ...
        List valid = ["dataobj", "fault", "enum", "spbm_do", "spbm_fault", "spbm_enum", "wsdl_do", "wsdl_enum", "wsdl_vimstub", "pyvmomi_audit", "pyvmomi_migrate", "pyvmomi_mo"]
        if (!(opt.type in valid)) {
            println "Invalid type detected. ${opt.type} not supported."
            cli.usage()
            System.exit(1)
        }
        if (opt.type != "pyvmomi_audit" && opt.type != "pyvmomi_migrate" && opt.type != "pyvmomi_mo" && !opt?.source) {
            println "--source is required for type ${opt.type}"
            cli.usage()
            System.exit(1)
        }
        if ((opt.type == "wsdl_vimstub" || opt.type == "pyvmomi_mo") && !opt?.dest) {
        ...
        if ((opt.type == "pyvmomi_audit" || opt.type == "pyvmomi_migrate") && !opt?.'yavijava-src') {
